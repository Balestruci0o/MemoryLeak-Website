...
const HeroSection = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // 3. OPTIMALIZOVANÝ POHYB - Pridaná kontrola pre dotykové zariadenia
  ...
  return (
    <section ref={sectionRef} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-background py-10">
      ...
      <div className="absolute inset-0 z-0 pointer-events-none">
        ...
        <div className="absolute bottom-0 right-1/4 w-72 h-72 md:w-96 md:h-96 bg-secondary/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: "1s" }} />
      </div>
      ...
      <div className="relative z-10 container mx-auto px-4 sm:px-6 text-center">
        ...
        <div className="hero-animate mb-6">
          <span className="inline-block px-4 py-2 text-[10px] md:text-xs font-mono font-semibold tracking-widest text-secondary border border-secondary/30 rounded-full uppercase">
            Digital Logic Education Platform
          </span>
        </div>

        <div className="relative inline-block mb-8">
          {/* MASKOT: Upravené pozicovanie pre veľmi úzke displeje (S20) */}
          <div 
            ref={mascotRef}
            className="absolute 
              -top-16 -right-6   /* Mobil */
              xs:-top-20 xs:-right-10
              md:-top-32 md:-right-24 /* Desktop */
              w-24 xs:w-32 md:w-48 lg:w-64 
              pointer-events-none will-change-transform"
            style={{ zIndex: -1 }}
          >
            <div className="absolute inset-0 bg-primary/20 blur-[40px] rounded-full" />
            <img 
              src="/assets/maskot_máva.png" 
              alt="Mascot" 
              className="w-full h-auto drop-shadow-[0_0_15px_rgba(0,255,255,0.4)] scale-x-[-1]"
            />
            <div 
              ref={bubbleRef}
              className="absolute -top-8 -left-8 bg-primary text-black font-mono text-[10px] px-2 py-1 rounded-md font-bold rotate-[-10deg] opacity-0 md:opacity-0"
            >
              01001000!
            </div>
          </div>

          {/* NADPIS: Použitie 'vw' jednotiek alebo lepšieho škálovania pre S20 */}
          <h1 className="hero-animate font-display text-[12vw] sm:text-7xl md:text-8xl lg:text-9xl font-black leading-none select-none tracking-tighter sm:tracking-normal">
            <span className="text-primary neon-text">MemoryLeak</span>
          </h1>
        </div>

        <p className="hero-animate max-w-2xl mx-auto text-base md:text-xl text-muted-foreground font-body mb-10 leading-relaxed px-2">
          Start with transistors, end with a working assembly-level computer. 
          <span className="block sm:inline"> Learn by building, not memorizing.</span>
        </p>
